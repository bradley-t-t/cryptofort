...
describe('Vault expiry', () => {
  ...
  // The restriction on characters above ASCII belongs to configuration values,
  // which are carried in request headers, and to nothing else. Credential names,
  // descriptions, and secrets are content: they travel as JSON, so a bullet or
  // an accent in one has to survive rather than be stripped in the name of the
  // header rule.
  it('keeps names, descriptions, and secrets that hold characters a header could not', async () => {
    await vault.put({
      name: 'facture-caf\u00E9 \u2022 2026',
      secret: 'sh\u0101red \u2022 v\u0430lue',
      description: 'Th\u00E9 caf\u00E9 account \u2022 renewed yearly',
    });
    expect(await vault.get('facture-caf\u00E9 \u2022 2026')).toBe('sh\u0101red \u2022 v\u0430lue');
    const [meta] = await vault.search('caf\u00E9');
    expect(meta.description).toBe('Th\u00E9 caf\u00E9 account \u2022 renewed yearly');
  });
