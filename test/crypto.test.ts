...
describe('Crypto', () => {
  ...
  it('produces a distinct IV each call', async () => {
    const c = new Crypto({ key });
    // Long enough that two ciphertexts of the same plaintext under different IVs
    // cannot collide by chance; a single byte collides once every 256 runs.
    const plaintext = 'x'.repeat(32);
    const a = await c.seal(plaintext);
    const b = await c.seal(plaintext);
